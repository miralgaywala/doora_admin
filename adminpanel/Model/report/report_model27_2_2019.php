...
class report_model
{
    ...
    public function gender_report($msg)
    {
      $con=$this->db->connection();
       ...
       if($msg == 'last 1 year')
       {
          $getbusinessuser=$con->query("SELECT count(user_id) as users , gender FROM users WHERE is_active=1 AND DATE_FORMAT(created_at, '%m-%Y') AND NOT gender = '' GROUP BY gender");
       }
       elseif ($msg == 'last 30 days') {
          $getbusinessuser=$con->query("SELECT count(user_id) as users , gender FROM users WHERE is_active=1 AND created_at > DATE_SUB(CURDATE(), INTERVAL 1 MONTH) AND NOT gender = '' GROUP BY gender");
       }
       elseif ($msg == 'last 7 days') {
          $getbusinessuser=$con->query("SELECT count(user_id) as users , gender FROM users WHERE is_active=1 AND created_at > DATE_SUB(CURDATE(), INTERVAL 7 DAY) AND NOT gender = '' GROUP BY gender");
       }
       else
       {
          $getbusinessuser=$con->query("SELECT count(user_id) as users , gender FROM users WHERE is_active=1 AND created_at > DATE_SUB(CURDATE(), INTERVAL 1 DAY) AND NOT gender = '' GROUP BY gender");
       }
       while ($row = $getbusinessuser->fetch_assoc()) {
        $businessuser[] = $row;
      }
    ...
    public function genderfilter_report()
    {
             $con=$this->db->connection();
             $category = array();
              $getcategory=$con->query("SELECT count(user_id) as users , gender FROM users WHERE is_active=1 AND DATE_FORMAT(created_at, '%m-%Y') AND NOT gender = '' GROUP BY gender");
               $category = array();
              while ($row = $getcategory->fetch_assoc()) {
                $category[] = $row;
              }
