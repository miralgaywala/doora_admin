...
class doora_dollor_terms_model
{
    ...
    public function add_doora_dollor_terms_model($doora_dollor_terms_title,$doora_dollor_terms_desc)
    {
      ...
      $add=$con->query("insert into doora_dollar_tearms (title,doora_dollar_tearms_desc,created_at,updated_at) values('".$doora_dollor_terms_title."','".$doora_dollor_terms_desc."','".$date."','".$date."')"); 

      $edittag="1";      
      return $edittag;
    }
    public function dete_doora_dollor_terms_model($id){
      $doora_dollor_terms_id=trim($id);
      $con= $this->db->connection();

      $date = gmdate("Y-m-d\TH:i:s\Z");

      $tag=$con->query("update doora_dollar_tearms SET is_deleted=1, updated_at='".$date."' where id=".$doora_dollor_terms_id); 
