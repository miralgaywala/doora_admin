    ...
    <input type="hidden" name="hid" class="form-control" value="<?php echo $row['id']; ?>">
      <div class="box-body">
        <div class="form-group">
          ...
          <input type="text" name="title" class="form-control" value="<?php echo $row['merchant_section_title']; ?>">
        </div>

        <div class="form-group">
          <label for="category">Description</label>
          <textarea id="editor2" name="desc" rows="10" cols="80"><?php echo $row['merchant_section_description']; ?></textarea>
        </div>

        <div class="form-group">
          <label for="category">Image</label>
          <input type="file" name="image1" class="form-control" value="<?php echo $row['merchant_section_image']; ?>" id="cimg1">
        </div>

        <div class="row">
          <div class="col-md-6">
            <?php
             if($row['merchant_section_image']=="")
             {
              ?>
             ...
             {
              ?>
                <img src="<?php echo IMAGE_SRC_PATH_NEW.$row['merchant_section_image']; ?>?<?=rand(1,32000)?>" width="80%"  class="cimg1">
              <?php
             }
            ?>
          </div>
        ...
        <div class="form-group">
          ...
          <input type="text" name="btnname" class="form-control" value="<?php echo $row['merchant_section_button_text']; ?>">
        </div>
...
function update()
{
  ...
  $.ajax({
      url : "website_editor/update_home_merchant_section.php",
      data:frdata,
      method:"post",
      processData:false,
      contentType:false,
