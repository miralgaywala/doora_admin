...
<section class="content-header" id="app_div1"> 
  <h4>Update Merchant Image Section</h4>
    ...
    <input type="hidden" name="hid" class="form-control" value="<?php echo $row['id']; ?>">
      <div class="box-body">
        <div class="form-group">
          ...
          <input type="file" name="image1" class="form-control" value="<?php echo $row['image_section_image']; ?>" id="cimg1">
        </div>

        <div class="row">
          <div class="col-md-6">
            <?php
             if($row['image_section_image']=="")
             {
              ?>
...
function update()
{
  ...
  $.ajax({
      url : "website_editor/update_merchant_image_section.php",
      data:frdata,
      method:"post",
      processData:false,
      contentType:false,
